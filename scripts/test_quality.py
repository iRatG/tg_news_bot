...
"""
scripts/test_quality.py — A/B тест Perplexity sonar-pro vs sonar.

Берёт реальные статьи из БД (уже опубликованные), прогоняет Writer и Formatter
с двумя моделями без публикации в Telegram. Сравнивает метрики качества и стоимость.

Использование:
    python scripts/test_quality.py          # 5 статей
    python scripts/test_quality.py --n 3    # 3 статьи
    python scripts/test_quality.py --n 7 --formatter  # 7 статей + тест форматтера
"""
